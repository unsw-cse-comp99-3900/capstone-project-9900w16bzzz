...
const ValidationPage = () => {
  ...
  const [invoice, setInvoice] = useState({ invoiceId: invoiceId, invoiceJsonVO: null });
  ...
  const loadPage = useCallback(async () => {
    try {
        ...
        const response = await fetch(`${endpoint}?invoiceId=${invoice.invoiceId}`, {
          method: 'GET',
          headers: {
    ...
    <div>
      <MainContainer id="main" style={{height: "auto"}}>
        ...
        {step === 0 && (
          ...
          >
            <InvoiceForm  goToStep={goToStep} invoice={invoice} setValidationResult={setValidationResult} type='validation' />
          </motion.div>
        )}
        {step === 5 && (
          <motion.div
