...
function InvoiceForm({
      ...
      const sectionsToExpand = sections
        .filter((section) =>
          section.fields.some((field) =>
            unfilledFields.some((unfilledField) =>
              unfilledField.startsWith(field)
            )
          )
        )
  ...
  const setDefaultValues = (data) => {
    ...
    newData.customizationId =
      "urn:oasis:company:specification:ubl:schema:xsd:Invoice-2";
    ...
    if (field === "dueDate" || field === "invoiceDate") {
      ...
      setFormData((prevData) => {
        ...
        if (keys[keys.length - 1] === "type" && keys.includes("allowance")) {
          ...
          keys[keys.length - 1] === "reason" &&
          keys.includes("allowance")
        ) {
          current["type"] =
            Object.keys(typeReasonMapping).find(
              (key) => typeReasonMapping[key] === value
            ) || "";
        }
        return newData;
      });
  ...
  const renderSection = (title, data) => {
    ...
    if (
      (title === "Payment" || title === "Delivery Address") &&
      isUBLStandardRuleSelected
    ) {
      return null;
    }

    const sectionHasUnfilledFields =
      showValidation &&
      Object.keys(data).some((key) =>
        unfilledFields.some(
          (field) => field.startsWith(`${key}.`) || field === key
        )
      );
  ...
  const sections = [
    ...
    { title: "Seller", fields: ["seller"] },
    {
      ...
      fields: ["subTotal", "invoiceTotal", "taxTotal"],
    },
  ...
  const optionMappings = {
    schemeId: [
      ...
      {
        value: "EORI",
        label: "Economic Operators Registration and Identification",
      },
      ...
      {
        value: "NACE",
        label: "European Standard Industry Classification Code",
      },
  ...
  const uploadEditedInvoice = async () => {
    ...
    if (formattedData.invoiceDate) {
      formattedData.invoiceDate = new Date(formattedData.invoiceDate)
        .toISOString()
        .split("T")[0];
    }
    ...
    try {
      ...
      setIsUploading(false);
      showPopup(
        `An error occurred while updating the invoice. Please try again. Error info: ${error}`,
        "error"
      );
      return;
    }
    ...
    try {
      ...
      setIsUploading(false);
      showPopup(
        `An error occurred while processing the file. Please try again. Error type: ${error}`,
        "error"
      );
    }
  };
      ...
      <MainContainer className="name">
        ...
        <Content>
          <HeaderContent>
            {isValidation ? (
              <h1>
                <span>Convert </span>to UBL
              </h1>
            ) : (
              <h1>
                <span>Last Step </span>Convert to UBL
              </h1>
            )}
            <p className="details">
              Additional information not shown on the invoice may be required.
            </p>
            <p className="details">
              Please select validation rules and check information correctness.
            </p>
            <CheckboxWrapper>
              <CheckboxInput
                options={[
                  {
                    value: "AUNZ_UBL_1_0_10",
                    label: "UBL Standard Rule 1.0.10",
                  },
                  {
                    value: "AUNZ_PEPPOL_1_0_10",
                    label: "Peppol Standard Rule 1.0.10",
                  },
                  {
                    value: "AUNZ_PEPPOL_SB_1_0_10",
                    label: "Peppol Small Business Rule 1.0.10",
                  },
                ]}
                  ...
                  onClick={() => {
                    if (selectedRules.length === 0) {
                      ...
                      const sectionsToExpand = sections
                        .filter((section) =>
                          section.fields.some((field) =>
                            emptyFields.some((emptyField) =>
                              emptyField.startsWith(field)
                            )
                          )
                        )
                        .map((section) => section.title);
        ...
        <ArrowButton
          disabled
          style={{ opacity: 0, cursor: "not-allowed", pointerEvents: "none" }}
        >
          <ArrowIcon style={{ transform: "scaleX(-1)" }} />
        </ArrowButton>
      </MainContainer>
